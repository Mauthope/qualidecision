...
export const CustomerProfileHeader: React.FC<Props> = ({ customer, complaints, concessions }) => {
  ...
  const handleSimulateWithAi = () => {
    openAiDrawer(`Qual é o perfil de tolerância, histórico de reclamações e recomendações técnicas para liberar lotes com desvio para o cliente ${customer.name}?`);
  };
            ...
            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {(customer.location || customer.cityState) && (
                  <span className="px-2.5 py-0.5 rounded-lg text-xs font-medium bg-cyan-950/30 text-cyan-300 border border-cyan-500/30 flex items-center gap-1">
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col items-start lg:items-end gap-3 shrink-0">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                title="Abrir Assistente IA com contexto deste cliente"
              >
                <Bot className="w-4 h-4 text-purple-200" />
                <span>Simular Decisão com IA</span>
              </button>

              <button
                onClick={() => setIsConcessionOpen(true)}
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-cyan-500 text-slate-950 hover:bg-cyan-400 transition-all cursor-pointer active:scale-95"
              >
